...
export default function ListSuratMasuk() {
  ...
  // Fetch surat masuk dari backend
  useEffect(() => {
              ...
              {loading ? (
                ...
                data.map((row, i) => (
                  <tr key={row._id} className="even:bg-gray-50">
                    ...
                    <td className="border border-gray-300 px-2 py-1">{row.recievedDate ? new Date(row.recievedDate).toLocaleDateString() : '-'}</td>
                    <td className="border border-gray-300 px-2 py-1">{row.number}</td>
                    <td className="border border-gray-300 px-2 py-1">{row.origin}</td>
                    ...
                    <td className="border border-gray-300 px-2 py-1 text-center">
                      <button
                        ...
                        title="Edit"
                        onClick={() => navigate(`/dashboard/view-surat/${row._id}?edit=1`)}
                      >
                        <svg className="w-5 h-5 inline" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 1 1 3 3L7 19.5 3 21l1.5-4L16.5 3.5z"/></svg>
                      </button>
                      <button
                      ...
                      >
                        <svg className="w-5 h-5 inline" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M3 6h18"/><path d="M8 6v12a2 2 0 0 0 2 2h4a2 2 0 0 0 2-2V6"/><path d="M19 6l-1.5 14a2 2 0 0 1-2 2H8.5a2 2 0 0 1-2-2L5 6"/></svg>
                      </button>
                    </td>
                  </tr>
                ))
